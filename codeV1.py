...
class EEG_GUI(QtWidgets.QMainWindow):
    ...
    def init_ui(self):
        ...
        self.band_labels = self.create_band_indicators()
        fft_layout.addWidget(self.band_labels)
    ...
    def create_band_indicators(self):
        """Create brain wave indicators"""
        container = QtWidgets.QWidget()
        container.setStyleSheet("background-color: rgba(20, 30, 50, 0.5); border-radius: 8px; padding: 10px;")
        ...
        for name, color in bands:
            band_widget = QtWidgets.QWidget()
            band_layout = QtWidgets.QVBoxLayout()
            
            label = QtWidgets.QLabel(name)
            label.setAlignment(Qt.Qt.AlignCenter)
            label.setStyleSheet(f"color: {color}; font-size: 10pt; font-weight: bold;")
            
            value = QtWidgets.QLabel("0%")
            value.setAlignment(Qt.Qt.AlignCenter)
            value.setStyleSheet(f"color: {color}; font-size: 14pt; font-weight: bold;")
            self.band_values.append(value)
            
            band_layout.addWidget(label)
            band_layout.addWidget(value)
    ...
    def update(self):
        ...
        # Band powers (update less frequently)
        if self.update_counter % 5 == 0:  # Update every 5th cycle
